...
Dim year_open, year_close, YearlyChange, PercentChange, TotalStockVolume As Double
...
For Each ws In Worksheets
    ...
    LastRow = ws.Cells(Rows.Count, "A").End(xlUp).Row
        ...
        For i = 2 To LastRow
            ...
            ws.Cells(start_data, 10).Value = YearlyChange
            ws.Cells(start_data, 11).Value = PercentChange

            'Use percentage format

            ws.Cells(start_data, 11).NumberFormat = "0.00%"
            ws.Cells(start_data, 12).Value = TotalStockVolume
